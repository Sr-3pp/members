  ...
  <section desk="members">
    <div class="tabs-component">
      <ul class="tabs">
        ...
        <li
          @click="$bus.$emit('registerModal', { sw: section, admin: 1 })"
          class="item action"
        >
      ...
      <div class="contents">
        <ul class="content">
          <ol v-for="(u, i) in list" :key="u.id" class="item">
            <span
              @click="toggleStatus(u.user_id, i)"
              class="status"
      ...
      <ul class="content" v-if="duser">
        <figure class="profile">
          <div class="actions">
            <button
              ...
              @click="edit ? (edit = false) : (edit = true)"
            >
          ...
          <label class="img" :class="{ edit: edit }">
            ...
            <img
              v-if="!edit"
              class="rango"
              :src="'/media/img/medallas/' + duser.rango + '.png'"
              alt=""
            />
            <input
              v-if="edit"
        ...
        <ul class="categorias">
          ...
          <label v-for="(c, ci) in categorias" class="img" :key="ci + 1">
            <input
              ...
              type="checkbox"
              name="categoria"
              v-model="duser.user.cats"
              :value="c.id"
              :true-value="c.id"
              :checked="
                duser.user.categorias
                  .map((e) => {
        ...
        <template v-for="(u, i) in duser">
          ...
          <ul class="idiomas item" v-if="i == 'idiomas'" :key="i + 1">
            ...
            <li class="item" v-for="(idioma, iid) in u" :key="iid + 1">
              ...
              <button
                v-if="edit"
                @click="duser.idiomas.splice(iid, 1)"
                class="btn btn-link"
              >
            ...
            <li v-if="edit" class="item list">
              ...
              <transition name="fade">
                <ul
                  v-if="idiomasResults && idioma !== ''"
                  class="list-group input-results"
                >
                  <button
                    @click="duser.idiomas.push(idioma.nombre)"
                    :key="idioma.id"
                    role="button"
        ...
        <ul class="valoraciones">
          ...
          <li
            class="item"
            :class="{ edit: edit }"
            v-for="(v, ind) in duser.user.valoracion"
            :key="ind + 1"
          >
            ...
            <span
              class="star"
              @click="edit ? (v.porcentaje = n) : null"
          ...
          <li class="item new" v-if="edit && nval.status">
            ...
            <div class="input stars">
              ...
              <span
                class="star"
                @click="nval.porcentaje = n"
                :class="{ active: n <= parseInt(nval.porcentaje) }"
                v-for="n in 10"
                :key="n"
              >
                <i class="fas fa-star"></i>
            ...
            <button
              ...
              @click="duser.user.valoracion.push(nval)"
            >
              agregar
            </button>
          </li>
          <li
            v-if="
              edit && duser.user.valoracion && duser.user.valoracion.length < 5
            "
            class="item"
          >
            <button
              class="btn btn-primary"
              @click="
                nval.status ? (nval.status = false) : (nval.status = true)
              "
            >
              <i class="fas fa-plus"></i>
            </button>
...
export default {
  ...
  methods: {
    ...
    userDetail(u) {
      ...
      this.duser = u;
      this.duser.user.cats = this.duser.user.categorias.map((e) => {
      ...
      .profile {
        ...
        .img {
          ...
          .user {
            ...
            overflow: hidden;
            object-fit: cover;
            box-shadow: 0rem 0.2rem 1rem rgba(#424242, 0.2);
            border: {
              style: solid;
              color: #fff;
              width: 0.5rem;
            ...
            input {
              ...
              &:checked {
                ~ img {
                  width: 3.5rem;
                  border-radius: 100%;
                  box-shadow: 0 0.2rem 0.2rem rgba(#383737, 0.3);
                }
              }
            }
            img {
