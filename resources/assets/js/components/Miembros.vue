<template>
  ...
  >
    <div
      ...
      :class="{ plusWidth: selected }"
    >
      <div v-if="selected" class="membersBackBtn">
        <button class="btn-back btn" @click="selected = false">
          < Regresar
        </button>
      </div>
      <div class="col-md-12 no-padding">
        <div v-if="members[0]">
          ...
          <h3 v-if="!selected" class="text-center">
            Selecciona el área de tu interes
          </h3>
              ...
              <div class="col-sm-4" style="position: relative">
                ...
                <transition name="fade">
                  <ul v-if="searchCountry" class="input-results list-group">
                    <a
                      @click="setPais(pais.id, pais.nombre)"
        ...
        <transition-group name="fade">
          <loader v-if="loader" key="0"></loader>
          <div
            v-if="selected && selected !== 5 && members[0]"
            id="memberCards"
            class="row mob-width"
            :style="active"
            key="1"
              ...
              >
                <div
                  ...
                  :class="{
                    conscard: consultor,
                    coachcard: coach,
                    capacard: capacitador,
                    empresacard: empresa,
                    programcard: programa,
                    especial: certificacion,
                  }"
                >
                  <div class="card-header">
                    <div class="row">
                      ...
                      <div class="col-md-9 text-right">
                          ...
                          <div class="col-md-12" style="padding-top:3px;">
                            <div class="card-category">
                              <span v-if="selected === 1">{{
                                $t("categoria.Consultor")
                              }}</span>
                              <span v-if="selected === 2">{{
                                $t("categoria.Coach")
                              }}</span>
                              <span v-if="selected === 3">{{
                                $t("categoria.Capacitador")
                              }}</span>
                              <span v-if="selected === 4">{{
                                $t("categoria.Empresa")
                              }}</span>
                              <span v-if="selected === 5">{{
                                $t("categoria.Programa")
                              }}</span>
                              <span v-if="selected === 6">{{
                                $t("categoria.CertificacionesEspecializadas")
                              }}</span>
                              &nbsp;&nbsp;&nbsp;
                              <img
                                v-if="selected === 1"
                            ...
                            <h3 v-if="selected !== 4" class="white">
                              {{ member.perfil.name }}
                              {{
                                member.perfil.apellido_p
                                  ? member.perfil.apellido_p
                                  : "" + " " + member.perfil.apellido_m
                                  ? member.perfil.apellido_m
                                  : ""
                              }}
                            </h3>
                            <h3 v-else class="white">
                              {{ member.empresa.name }}
                            </h3>
                          </div>
                        </div>
                        <a
                          v-if="
                            (member.perfil && member.perfil.fb) ||
                              (member.empresa && member.empresa.fb)
                          "
                          :href="
                            member.perfil
                              ? 'https://facebook.com/' + member.perfil.fb
                              : 'https://facebook.com/' + member.empresa.fb
                          "
                          ><img
                            width="30"
                            src="/media/img/recursos/fb-logo.png"
                            alt="facebook logo"
                        /></a>

                        <a
                          v-if="
                            (member.perfil && member.perfil.tw) ||
                              (member.empresa && member.empresa.tw)
                          "
                          :href="
                            member.perfil
                              ? 'https://twitter.com/' + member.perfil.tw
                              : 'https://twitter.com/' + member.empresa.tw
                          "
                          ><img
                            width="30"
                            src="/media/img/recursos/tw-logo.png"
                            alt="twitter logo"
                        /></a>

                        <a
                          v-if="
                            (member.perfil && member.perfil.in) ||
                              (member.empresa && member.empresa.in)
                          "
                          :href="
                            member.perfil
                              ? 'https://linkedin.com/' + member.perfil.in
                              : 'https://linkedin.com/' + member.empresa.in
                          "
                          ><img
                            width="30"
                            src="/media/img/recursos/in-logo.png"
                            alt="linkedin logo"
                        /></a>

                        <a
                          v-if="
                            (member.perfil && member.perfil.insta) ||
                              (member.empresa && member.empresa.insta)
                          "
                          :href="
                            member.perfil
                              ? 'https://instagram.com/' + member.perfil.insta
                              : 'https://instagram.com/' + member.empresa.insta
                          "
                          ><img
                            width="30"
                            src="/media/img/recursos/in-logo.png"
                  ...
                  <div class="card-body">
                    <div class="card-text text-center">
                      <table class="table">
                        ...
                        <tr>
                          ...
                          <td class="text-left">
                            {{
                              member.perfil
                                ? member.perfil.website
                                : member.empresa.website
                            }}
                          </td>
                        </tr>
                      </table>
                      ...
                      <a
                        ...
                        :href="
                          '/perfil/' + member.folio + '/' + member.perfil.slug
                        "
                        >Más información</a
                      >
                      <a
          ...
          <div
            v-else-if="selected && selected !== 5 && !members[0]"
            id="memberCards"
            class="row mob-width no-members"
            key="1"
          >
            <h2>
                ...
                <div class="card-header">
                  <div class="row">
                    ...
                    <div class="col-sm-9 text-right">
                      ...
                      <a v-if="program.empresa.fb" :href="program.empresa.fb"
                        ><img
                          width="30"
                          src="/media/img/recursos/fb-logo.png"
                          alt="facebook logo"
                      /></a>
                      <a v-if="program.empresa.tw" :href="program.empresa.tw"
                        ><img
                          width="30"
                          src="/media/img/recursos/tw-logo.png"
                          alt="twitter logo"
                      /></a>
                      <a v-if="program.empresa.in" :href="program.empresa.in"
                        ><img
                          width="30"
                          src="/media/img/recursos/in-logo.png"
                          alt="linkedin logo"
                      /></a>
                      <a
                        v-if="program.empresa.insta"
                        :href="program.empresa.insta"
                        ><img
                          width="30"
                          src="/media/img/recursos/in-logo.png"
                          alt="instagram logo"
                    ...
                    <a
                      ...
                      :href="
                        '/programa/' +
                          program.id +
                          '/' +
                          program.nombre.toLowerCase().replace(/\s/g, '-')
                      "
                      >Más información</a
                    >
                  </div>
